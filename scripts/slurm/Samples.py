...
for k in range(1, 1001): #101
    dfptX= pd.read_csv('/home/mvasist/simulations_new/16_params/X_'+ str(k) + '.csv', low_memory = False) #chunksize=1000, iterator=True, dtype={'col2': np.float32}
    dfT= pd.read_csv('/home/mvasist/simulations_new/16_params/T_'+ str(k) + '.csv', low_memory = False)
    ptX.append(dfptX.values) #for x,t in zip(dfX_M,dfT_M):
    T.append(dfT.values)
...
ptXframe = pd.DataFrame(comb_np_array_ptX)
Tframe = pd.DataFrame(comb_np_array_T)

list_of_tensors_ptX = [torch.tensor(np.array(ptXframe),dtype=torch.float32)]
list_of_tensors_T = [torch.tensor(np.array(Tframe),dtype=torch.float32)] #torch.tensor

ptXX = torch.cat(list_of_tensors_ptX)[:, 1:]
TT = torch.cat(list_of_tensors_T)[:,1:]

XX = ptXX[:][947*2:] # and temp =  ptXX[:][947:154+947] and pressure =  ptXX[:][:154]

# wlen = torch.load('wlen.pt')

#################################################################################################################

#1. training architeture insert here OR 
...
observation = torch.load('/home/mvasist/scripts/observations/...pt') 
...
with open('../time/16params/time_'+'howmanysamples'+'.csv', 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the data
